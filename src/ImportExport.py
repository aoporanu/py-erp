...
class ImportCsv(object):
    ...
    def ProductImport(self, importfile):
        ...
        if self.isdonme == True:
            ...
            return None
        return showinfo("Successfull",
                        "All producs Have been Imported!!\nBetter save it! if you want to keep the change.")
    ...
    def ProductImportGui(self, C_value):
        ...
        fg = 'White'
        ############################################################################################
        Label(app, text="Name", background=colour, foreground=fg, font=F.Font(family='Times', size=15)).grid(row=0,
                                                                                                             column=0,
                                                                                                             sticky=N
                                                                                                                    +
                                                                                                                    W
        ...
        Label(app, text='Category', background=colour, foreground=fg, font=F.Font(family='Times', size=15)).grid(row=1,
                                                                                                                 column=0,
                                                                                                                 sticky=N + W + E + S,
                                                                                                                 padx=10,
                                                                                                                 pady=10)
        ...
        Label(app, text='Description', background=colour, foreground=fg, font=F.Font(family='Times', size=15)).grid(
            row=2, column=0, sticky=N + W + E + S, padx=10, pady=10)
    ...
    def CustomerImportGui(self, C_value):
        ...
        for row in maxrow:
            ...
            for column in maxcolumn:
                ...
                if (row == 0 and column == 1) or (row == 1 and column == 1) or (row == 2 and column == 1) or (
                        row == 3 and column == 1):
                    entry = Combobox(app, width=Combo_Width, value=C_value)
                    entry.grid(row=row, column=column, sticky=N + W + E + S, padx=10, pady=10)
                    self.el[row] = entry
                elif row == 0 and column == 0:
                    Label(app, background=colour, foreground=fg, text="Customer Name",
                          font=F.Font(family='Times', size=15)).grid(row=row, column=column, sticky=N + W + E + S,
                                                                     padx=10, pady=10)
                elif row == 1 and column == 0:
                    Label(app, background=colour, foreground=fg, text="Customer Phone",
                          font=F.Font(family='Times', size=15)).grid(row=row, column=column, sticky=N + W + E + S,
                                                                     padx=10, pady=10)
                elif row == 2 and column == 0:
                    Label(app, background=colour, foreground=fg, text="Customer Address  ",
                          font=F.Font(family='Times', size=15)).grid(row=row, column=column, sticky=N + W + E + S,
                                                                     padx=10, pady=10)
                elif row == 3 and column == 0:
                    Label(app, background=colour, foreground=fg, text="Customer Email",
                          font=F.Font(family='Times', size=15)).grid(row=row, column=column, sticky=N + W + E + S,
                                                                     padx=10, pady=10)
...
class ExportCsv(object):
    ...
    def writecustomer(self, exportfile):
        ...
        rows = self.db.sqldb.execute(
            """ SELECT customer_name,customer_address,customer_email,phone_no FROM customers JOIN contacts USING (customer_id) """).fetchall()
        for row in rows:
            writer.writerow(row)
        wri.close()
        return None
    ...
    def writeproducts(self, exportfile):
        ...
        writer.writerow(fields)
        rows = self.db.sqldb.execute(
            """ SELECT product_name,product_description,category_name FROM products JOIN category USING (category_id) """).fetchall()
        for row in rows:
            writer.writerow(row)
        wri.close()
        return 1
