...
class ADDInvoice():
    def __init__(self, master, db):
        ...
        Label(self.f, text="Invoice List", font=('Berlin Sans FB Demi', 21), foreground="#3496ff").grid(row=0, column=0,
                                                                                                        sticky=N + S
                                                                                                               + E + W,
                                                                                                        columnspan=2,
                                                                                                        padx=10, pady=5)
        self.mlb1 = MultiListbox(self.f, (
        ("Invoice ID", 40), ('Invoice No', 17), ('Invoice Date', 40), ('Customer Attached', 40)), height=15)
    ...
    def Insert(self):
        lists = self.db.sqldb.execute(""" SELECT invoice_id,invoice_no,invoice_date,customer_name FROM invoices JOIN 
    ...
    def Add_Invoice(self, edit=False):
        ...
        self.app['bg'] = cor
        Label(self.app, text=heading, font=('Berlin Sans FB Demi', 21), foreground="#3496ff").grid(row=0, column=0,
                                                                                                   sticky=N + S + E + W,
                                                                                                   padx=10, pady=5)
        self.t = Frame(self.app)
        self.t.grid(row=1, column=0, sticky=N + S + E + W)
        Label(self.t, text="Invoice No", background=cor, foreground=fg).grid(row=0, column=0, sticky=N + W + S + E,
                                                                             padx=10, pady=10)
        entry0 = Spinbox(self.t, from_=0, to=9999)
        entry0.grid(row=0, column=1, sticky=N + W + S + E, padx=10, pady=10)

        Label(self.t, text="Invoice Date", background=cor, foreground=fg).grid(row=1, column=0, sticky=N + W + S + E,
                                                                               padx=10, pady=10)
        cb0 = CalendarButton(self.t)
        cb0.grid(row=1, column=1, sticky=N + W + S + E, padx=10, pady=10)
        Label(self.t, text="Customer Name", background=cor, foreground=fg).grid(row=2, column=0, sticky=N + W + S + E,
                                                                                padx=10, pady=10)
        ...
        Label(self.t, text=" Customer Address  ", background=cor, foreground=fg).grid(row=3, column=0, sticky=N + W + E,
                                                                                      padx=10, pady=10)
        ...
        Label(self.t, text=" Customer Phone", background=cor, foreground=fg).grid(row=4, column=0, sticky=N + W + S + E,
                                                                                  padx=10, pady=10)
        ...
        Label(self.t, text="Amount", background=cor, foreground=fg).grid(row=0, column=2, sticky=N + W + S + E, padx=10,
                                                                         pady=10)
        entry4 = Entry(self.t, width=30)
        entry4.grid(row=0, column=3, sticky=N + W + S + E, padx=10, pady=10)
        Label(self.t, text="Paid", background=cor, foreground=fg).grid(row=1, column=2, sticky=N + W + S + E, padx=10,
                                                                       pady=10)
        entry5 = Entry(self.t, width=30)
        entry5.grid(row=1, column=3, sticky=N + W + S + E, padx=10, pady=10)
        Label(self.t, text="Discount", background=cor, foreground=fg).grid(row=2, column=2, sticky=N + W + S + E,
                                                                           padx=10, pady=10)
        drop = map(lambda x: x[0],
                   self.db.sqldb.execute(""" SELECT product_name FROM products ORDER BY product_name """).fetchall())
        ...
        Label(self.t, text="Products", background=cor, foreground=fg).grid(row=4, column=2, sticky=N + W + S + E,
                                                                           padx=10, pady=10)
        ...
        Label(tmpff, text="QTY", background=cor, foreground=fg).grid(row=0, column=1, sticky=N + W + S + E, padx=10,
                                                                     pady=10)
        ...
        def autofill(event):
            ...
            l = self.db.sqldb.execute("""SELECT customer_address,phone_no FROM customers JOIN contacts USING (
            customer_id)
        ...
        if edit == True:
            ...
            row = self.db.sqldb.execute("""SELECT product_name,QTY,price,cost_id FROM sells JOIN costs USING (
            cost_id) JOIN products USING(product_id)
