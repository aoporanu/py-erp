...
class ADDInvoice():
    ...
    def Insert(self):
        lists = self.db.sqldb.execute(
            """ SELECT invoice_id,invoice_no,invoice_date,customer_name FROM invoices JOIN
    ...
    def Add_Invoice(self, edit=False):
        ...
        def autofill(event):
            ...
            l = self.db.sqldb.execute(
                """SELECT customer_address,phone_no FROM customers JOIN contacts USING (
            customer_id)
        ...
        if edit == True:
            ...
            autofill(1)
            amount = 0.0
            paid = self.db.sqldb.get_cell("invoices", "invoice_id", "paid",
                                          tup[0])
            row = self.db.sqldb.execute(
                """SELECT product_name,QTY,price,cost_id FROM sells JOIN costs USING (
