...
class CalendarButton(Frame):
    def __init__(self, master, **kw):
        ...
        self.btn = Button(self, textvariable=self.datevar, command=lambda: self.opencalender(), takefocus=False)
    ...
    def update(self):
        date = datetime.datetime(int(self.year), int(self.month), int(self.day), int(self.hour), int(self.min),
                                 int(self.sec))
        st = date.ctime()
        self.datevar.set("")
        self.datevar.set(st)
    ...
    def opencalender(self):
        ...
        self.rootc1.wm_attributes("-alpha", 0.3)
